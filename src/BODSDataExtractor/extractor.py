...
# =============================================================================
        ...
        if self.stop_level:
            output_df.columns = ['URL', 'FileName', 'NOC', 'TradingName', 'LicenceNumber', 'OperatorShortName', 'OperatorCode', 'ServiceCode', 'LineName', 'PublicUse','OperatingDays', 'Origin', 'Destination', 'OperatingPeriodStartDate', 'OperatingPeriodEndDate', 'SchemaVersion', 'RevisionNumber','la_code','journey_pattern_json', 'vehicle_journey_json','services_json','stops_json']
        else:
...
# =============================================================================
    ...
    def extract_common_name(self, StopPointRef):
        ...
        stop_location = self.stop_object.AnnotatedStopPointRef[self.stop_point_index[StopPointRef]].Location
        ...
        if not stop_location:
            ...
            stop_lat = self.stop_object.AnnotatedStopPointRef[self.stop_point_index[StopPointRef]].Location.Latitude
    ...
    def next_jptl_in_sequence(self,jptl, vj_departure_time, vj, vjtl_index, first_jptl=False):
        ...
        if first_jptl:
            common_name, latitude, longitude = self.extract_common_name(jptl.From.StopPointRef)
    ...
    def map_indicies(self):
        ...
        self.journey_pattern_index = {key.id: value for value, key in enumerate(self.service_object.StandardService.JourneyPattern)}
        self.journey_pattern_section_index = {key.id: value for value, key in enumerate(self.journey_pattern_section_object.JourneyPatternSection)}
    ...
    def organise_timetables(self, collated_timetable_outbound, collated_timetable_inbound):
        ...
        if not self.collated_timetable_outbound.empty:
            ...
            self.collated_timetable_outbound.iloc[:, 5:] = self.collated_timetable_outbound.iloc[:, 5:].iloc[:, ::-1].values
    
            # ensuring the sequence numbers are sorted in ascending order
            self.collated_timetable_outbound.iloc[4:] = self.collated_timetable_outbound.iloc[4:].sort_values(by="Sequence Number",
                                                                                                    ascending=True)
        ...
        if not self.collated_timetable_inbound.empty:
            ...
            # ensuring the vjs times are sorted in ascending order
            self.collated_timetable_inbound.iloc[:, 5:] = self.collated_timetable_inbound.iloc[:, 5:].iloc[:, ::-1].values
    
            # ensuring the sequence numbers are sorted in ascending order
            self.collated_timetable_inbound.iloc[4:] = self.collated_timetable_inbound.iloc[4:].sort_values(by="Sequence Number",
                                                                                                  ascending=True)
